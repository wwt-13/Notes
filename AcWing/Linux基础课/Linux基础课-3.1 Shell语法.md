...
5. `$$`:表示脚本当时运行的进程
6. `$?`:表示上一条命令的退出状态，（注意不是stdout，而是exit code）。0表示正常退出，其他值表示错误，就像c++中的`return 0`
7. `$(command)`:很常用，返回command这条命令的stdout(输出结果)
...
> expr...是命令形式的，存在stdout和exit code，而'expr...'则纯粹是一个输出，比方说'expr 3 \\> 3'就在指令中就纯粹等于0
...
- `-p`:后面可以接提示信息`read -p "my name is:" name`
- `-t`:后面跟秒数，定义输入字符的等待时间，超过等待时间自动忽略此命令
...
> 和C/C++中几乎完全一模一样，所以直接看案例即可，参数之间用空格隔开
...
- test:exitcode，0表示真，1表示假（等价于return）
- 凡是进程状态表示结果的，都是0表示真
...
3. 首先top，查看当前正在运行的所有进程，找到死循环进程对应的pid，运行指令`kill -9 pid`即可
   ...
   plus:可以使用shift+m使得进程按照内存排序（不然内存老是跳来跳去的比较麻烦）

### 函数

> bash中的函数类似于C/C++中的函数，但是return的返回值与C/C++中的不同，返回的时exit code，取值为0-255，0表示正常结束
