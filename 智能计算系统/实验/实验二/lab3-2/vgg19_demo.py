    ...
    def build_model(
        ...
        for i in range(0, len(input_params), 2):
            self.input_quant_params.append(
                pycnml.QuantParam(int(input_params[i]), float(input_params[i + 1]))
            )
        for i in range(0, len(filter_params), 2):
            self.filter_quant_params.append(
                pycnml.QuantParam(int(filter_params[i]), float(filter_params[i + 1]))
            )
        ...
        self.net.createConvLayer(
            "conv4_3", 512, 3, 1, 1, 1, self.input_quant_params[10]
        )
        # relu4_3
        self.net.createReLuLayer("relu4_3")
        # conv4_4
        self.net.createConvLayer(
            "conv4_4", 512, 3, 1, 1, 1, self.input_quant_params[11]
        )
        # relu4_4
        ...
        # conv5_1
        self.net.createConvLayer(
            "conv5_1", 512, 3, 1, 1, 1, self.input_quant_params[12]
        )
        # relu5_1
        self.net.createReLuLayer("relu5_1")
        # conv5_2
        self.net.createConvLayer(
            "conv5_2", 512, 3, 1, 1, 1, self.input_quant_params[13]
        )
        # relu5_2
        self.net.createReLuLayer("relu5_2")
        # conv5_3
        self.net.createConvLayer(
            "conv5_3", 512, 3, 1, 1, 1, self.input_quant_params[14]
        )
        # relu5_3
        self.net.createReLuLayer("relu5_3")
        # conv5_4
        self.net.createConvLayer(
            "conv5_4", 512, 3, 1, 1, 1, self.input_quant_params[15]
        )
        # relu5_4
