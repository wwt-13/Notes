...
class ConvolutionalLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        self.input_col = img2col(
            self.input_pad, height_out, width_out, self.kernel_size, self.stride
        )
        self.weights_col = self.weight.transpose(3, 0, 1, 2).reshape(
            self.weight.shape[-1], -1
        )
        output = np.matmul(
            self.weights_col,
            self.input_col.reshape(
                self.input_col.shape[0], -1, self.input_col.shape[3]
            ),
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff_col = np.matmul(
            self.weights_col.T,
            top_diff.transpose(1, 2, 3, 0).reshape(self.channel_out, -1),
        )
        bottom_diff_col = bottom_diff_col.reshape(
            bottom_diff_col.shape[0], -1, self.input.shape[0]
        ).transpose(2, 0, 1)
        bottom_diff = col2img(
            bottom_diff_col,
            height_pad,
...
class MaxPoolingLayer(object):
                ...
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        self.max_index[
                            idxn,
                            idxc,
                            idxh * self.stride + curren_max_index[0],
                            idxw * self.stride + curren_max_index[1],
                        ] = 1
        return self.output
    ...
    def forward_speedup(self, input):
        ...
        self.input_col = img2col(
            self.input, self.height_out, self.width_out, self.kernel_size, self.stride
        ).reshape(
            self.input.shape[0],
            self.input.shape[1],
            -1,
    ...
    def backward_speedup(self, top_diff):
        ...
        # _______________________
        pool_diff = (self.max_index * top_diff[:, :, np.newaxis, :, :]).reshape(
            self.input.shape[0], -1, self.height_out * self.width_out
        )
        bottom_diff = col2img(
            pool_diff,
            self.input.shape[2],
