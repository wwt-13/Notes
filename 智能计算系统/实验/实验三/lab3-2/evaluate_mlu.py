...
def ffwd(data_in, paths_out, model, device_t="", batch_size=1):
    ...
    # ______________________
    img_shape = (
        get_img(data_in[0], (256, 256, 3)).shape if is_paths else data_in[0].shape
    )
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(num_iters):
                ...
                if is_paths:
                    X = [
                        get_img(path, img_shape)
                        for path in data_in[pos : pos + batch_size]
                    ]
                else:
...
def ffwd_different_dimensions(in_path, out_path, model, device_t=DEVICE, batch_size=4):
