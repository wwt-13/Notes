...
注：==以下功能前备注（超级管理员模式）为仅可以在超级管理员模式下执行的命令，（。模式）为仅可以在标准模式下执行的命令，未标注的命令在两种模式下均可执行==。
...
- （超级管理员模式）添加和删除线路
  ...
  - 异常处理
    ...
    - 添加线路时**同一线路上**的的两个站名不得重复（没有环线），否则不做任何修改并输出`Station duplicate` 
...
- （超级管理员模式）添加和删除车站
  ...
  - 异常处理
    ...
    - 添加车站时里程数应当符合规范，否则输出`Arguments illegal`。（这里的argument是首先输出的所以不用管？）
